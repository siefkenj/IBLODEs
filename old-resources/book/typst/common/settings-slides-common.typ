#import "./defaults.typ": config, env_lookup_all, restrict
...
#let create_slides_env(bottom_margin_color: color.rgb("#00647d")) = {
  ...
  let slide_title_box = color.rgb("#00647d")
  ...
  let slide(
    ...
    force_scale: none,
    bottom_margin_color: bottom_margin_color,
  ) = {
    ...
    set page(
      header: {
        place(
          top + left,
          dx: -x_margin,
          box(width: 100% + 2 * x_margin, height: 100% + 3pt, fill: slide_title_box),
        )
      ...
      layout(size => {
        ...
        if content_dim.height < 1.0 * size.height and not force_two_column {
          ...
          if percent_over < 1 {
            ...
            let text_size = if force_scale != none { force_scale } else {
