...
\begin{SaveDefinition}[key=ClassificationOfEquilibria, title={Classification of Equilibria}]
		...
		\begin{itemize}
			...
			\item \emph{stable}\index[definitions]{equilibrium!stable} if there is a fixed distance so that locally, solutions within that fixed distance of $f$
...
\begin{SaveDefinition}[key=ClassificationOfEquilibriaFormal, title={Classification of Equilibria (Formal)}]
		...
		\begin{itemize}
			...
			\item \emph{unstable at time $t_0$}\index[definitions]{equilibrium!unstable} if $f$ is not stable
		\end{itemize}
