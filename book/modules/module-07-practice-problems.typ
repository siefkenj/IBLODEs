...
#let questions = (
  ...
  (
    statement: [
      Find the solution $arrow(s)(t) = arrow(v)(t) + arrow(p)$ where $arrow(p)$ is a constant vector, and $arrow(v)(t)$ is a solution to the affine equation $arrow(v)'(t) = M arrow(v)(t) + arrow(b)$. Then find the equilibrium point $arrow(p)_("eq")$ of the affine equation, and describe how it compares to $arrow(p)$:
        ...
        + $M = mat(2, 0; 0, 5), quad arrow(b) = mat(6; -10)$
        + $M = mat(1, 1; 0, 2), quad arrow(b) = mat(3; 4)$
        + $M = mat(2, 1; 1, 1), quad arrow(b) = mat(0; 3)$
        + $M = mat(3, 1; 2, 1), quad arrow(b) = mat(4; 1)$  
    ],
    solution: [
      In each case $arrow(p) = M^(-1) arrow(b)$ and $arrow(s)(t) = arrow(v)(t) + arrow(p)$. Further, for a $2 times 2$ matrix with a non-zero determinant, $M^(-1) = 1/(det M)mat(d, -b; -c, a)$ where $a$, $b$, $c$, and $d$ are the entries of $M$.
  ...
  (
    statement: [
      ...
      #[
        ...
        + $M = mat(0, 1; 1, 0), quad arrow(b) = mat(2; -2)$
        + $M = mat(1, 2; 0, 3), quad arrow(b) = mat(-2; -3)$
        + $M = mat(0, 1; 2, 0), quad arrow(b) = mat(2; 2)$
        + $M = mat(-2, 1; 1, -2), quad arrow(b) = mat(0; 3)$
      ]
    ],
    solution: [
      ...
      #[
        #set enum(numbering: "(i)")
        + Equilibrium: $M arrow(p)_("eq") = -arrow(b) = mat(-2; 2)$ gives $arrow(p)_("eq") = mat(2; -2)$. The matrix has eigenvalues $lambda = 1, -1$ with eigenvectors $mat(1; 1)$ and $mat(1; -1)$, so
          $
            arrow(u)(t) = A e^(t) mat(1; 1) + B e^(-t) mat(1; -1)
          $
          and therefore
          $
          ...
          $
          $
            &= A e^(t) mat(1; 1) - B e^(-t) mat(1; -1) + mat(-2; 2) + mat(2; -2) \
            &= A e^(t) mat(1; 1) - B e^(-t) mat(1; -1).
          $
          The two agree, so $arrow(s)$ is a solution.

        + $arrow(p)_("eq") = mat(0; 1)$. Eigenvalues $lambda = 1, 3$ with eigenvectors $mat(1; 0)$ and $mat(1; 1)$, so
          $
            arrow(s)(t) = A e^(t) mat(1; 0) + B e^(3t) mat(1; 1) + mat(0; 1) \ "where" A "and" B "are parameters."
          $

        + $arrow(p)_("eq") = mat(-1; -2)$. Eigenvalues $lambda = plus.minus sqrt(2)$ with eigenvectors $mat(1; sqrt(2))$ and $mat(1; -sqrt(2))$, so
          $
            arrow(s)(t) = A e^(sqrt(2) t) mat(1; sqrt(2)) + B e^(-sqrt(2) t) mat(1; -sqrt(2)) - mat(1; 2) \ "where" A "and" B "are parameters."
          $

        + $arrow(p)_("eq") = mat(1; 2)$. Eigenvalues $lambda = -1, -3$ with eigenvectors $mat(1; 1)$ and $mat(-1; 1)$, so
          $
  ...
  (
    statement: [
      ...
      + Reconstruct $M$. (Recall $M = P D P^(-1)$, where the columns of $P$ are the eigenvectors and $D$ is the diagonal matrix of the corresponding eigenvalues.)
      ...
      #[
        #set enum(numbering: "(i)")
          + $ arrow(s)(t) = A e^(2t) mat(1; 0) + B e^(-t) mat(0; 1) + mat(3; 1) $
          + $ arrow(s)(t) = A e^(3t) mat(1; 1) + B e^(t) mat(1; -1) + mat(0; 2) $
          + $ arrow(s)(t) = A e^(-2t) mat(1; 0) + B e^(t) mat(1; 1) + mat(1; -1) $
          + $ arrow(s)(t) = A e^(t) mat(2; 1) + B e^(-t) mat(1; 1) + mat(-1; 1) $
      ]
    ],
    solution: [
      ...
      #[
        #set enum(numbering: "(i)")
          + The eigenpairs are $(2, mat(1; 0))$ and $(-1, mat(0; 1))$. Since the eigenvectors are the standard basis, $M$ is simply diagonal:
            $
              M = mat(2, 0; 0, -1).
            $
          ...
          + $M = mat(2, 1; 1, 2)$ (eigenpairs $(3, mat(1; 1))$ and $(1, mat(1; -1))$), with $arrow(p)_("eq") = mat(0; 2)$, so $arrow(b) = mat(-2; -4)$.

          + $M = mat(-2, 3; 0, 1)$ (eigenpairs $(-2, mat(1; 0))$ and $(1, mat(1; 1))$), with $arrow(p)_("eq") = mat(1; -1)$, so $arrow(b) = mat(5; 1)$.

          + $M = mat(3, -4; 2, -3)$ (eigenpairs $(1, mat(2; 1))$ and $(-1, mat(1; 1))$), with $arrow(p)_("eq") = mat(-1; 1)$, so $arrow(b) = mat(7; 5).$
      ]
    ],
  ),
  (
    statement: [
      ...
      #[
        #set enum(numbering: "(i)")
          + $ M = mat(0, 1; 1, 0), quad arrow(b) = mat(2; -2), quad arrow(r)(0) = mat(4; 2) $
          + $ M = mat(1, 2; 0, 3), quad arrow(b) = mat(-2; -3), quad arrow(r)(0) = mat(4; 2) $
          + $ M = mat(-2, 1; 1, -2), quad arrow(b) = mat(0; 3), quad arrow(r)(0) = mat(2; 5) $
          + $ M = mat(2, 3; 0, -1), quad arrow(b) = mat(2; 1), quad arrow(r)(0) = mat(1; 0) $
      ]
    ],
    solution: [
      ...
      #[
        #set enum(numbering: "(i)")
          + See the previous problem for the equilibrium and general solution.
          ...
          + $arrow(p)_("eq") = mat(0; 1)$, eigenpairs $(1, mat(1; 0))$ and $(3, mat(1; 1))$. The condition $A mat(1; 0) + B mat(1; 1) = mat(4; 2) - mat(0; 1) = mat(4; 1)$ gives $B = 1$, $A = 3$, so
            $
              arrow(s)(t) = 3 e^(t) mat(1; 0) + e^(3t) mat(1; 1) + mat(0; 1).
            $

          + $arrow(p)_("eq") = mat(1; 2)$, eigenpairs $(-1, mat(1; 1))$ and $(-3, mat(-1; 1))$. The condition $A mat(1; 1) + B mat(-1; 1) = mat(2; 5) - mat(1; 2) = mat(1; 3)$ gives $A = 2$, $B = 1$, so
            $
              arrow(s)(t) = 2 e^(-t) mat(1; 1) + e^(-3t) mat(-1; 1) + mat(1; 2).
            $

          + The matrix is upper-triangular, so the eigenvalues are $lambda = 2, -1$ with eigenvectors $mat(1; 0)$ and $mat(-1; 1)$. For the equilibrium, $M arrow(p)_("eq") = -arrow(b) = mat(-2; -1)$. Thus $arrow(p)_("eq") = mat(-5/2; 1)$ and
            $
  ...
  (
    ...
    solution: [
      ...
      + Rearranging, every affine solution can be written $arrow(s)_1 = arrow(s)_2 + arrow(w)$, where $arrow(w)$ is some solution of the matrix equation. Fixing one particular affine solution $arrow(s)_p$, the general affine solution is $arrow(s)_p$ plus the general solution of the matrix equation. Geometrically, the affine solution set is the matrix-equation solution set translated by $arrow(s)_p$.
    ],
  ),
  (
    statement: [
  ...
  (
    statement: [
      *(When centering fails.)* Centering relies on the equilibrium $arrow(p)_("eq") = -M^(-1) arrow(b)$, which requires $M$ to be invertible. Suppose instead that $0$ is an eigenvalue of $M$.
      ...
      + Let $M = mat(1, 1; 1, 1)$ and $arrow(b) = mat(1; -1)$. Show there is no equilibrium, then describe what solutions do along the $lambda = 0$ direction.
    ],
    solution: [
