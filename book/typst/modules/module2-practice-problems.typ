...
#let questions = (
  (
    statement: [
      On a hand written graph that has the domain
      $D = (-4,4)$
      and range
      $R = (-4,4)$
      draw a slope field of the following differential equations, with a slope at all coordinates
      $(n,m)$, with $n,m in ZZ$ being integers.
      + $P'(t) = P(t)$
      + $y y' = x$ where $x$ is the dependent variable.
      + $P'(t) = P(t) - t + 1$
      + $H'(u) + u H(u) = cos(H(u))$
    ],
    solution: [
  ...
  (
    statement: [
      For each of the slope fields that you drew in question 1, sketch on the slope field the
      solution to the differential equation that goes through the point $(0,1)$.
    ],
  ),
  (
    statement: [
      ...
      $
        (dif T)/(dif x) = 10T - T^2
      $
      and initial condition $(x_0,T_0) = (0,2)$, fill in using Euler's the following table, or make
      a filled in copy of it on a separate paper. #align(
        center,
        block(
          breakable: false,
          simple_table(
  ...
  (
    statement: [
      Use Excel, and Euler's method with the given step size $Delta$ and initial condition
      $(t_0,y_0)$, to find an approximation $y_1$ of a solution $y$ to the given differential
      equation at the given point $t_1 = underline("   ")$. Make sure you also make a graph in Excel
      too! (The $y$ label is replaced with $P$ in part (c))
      + $y'(t) = - y(t)$, $Delta = 0.1$, $t_0 = 0$, $y_0 = 50$, $t_1 = 5$, $y_1 = " " ?$.
      + $y'(t) = y(t) sin(t)$, $Delta = 0.01 pi$, $t_0 = 0$, $y_0 = 2$, $t_1 = 2pi$, $y_1 = " " ?$.
      + $P'(t) = P(t) - t + 1$, $Delta = 1$, $t_0 = 0$, $P_0 = 1$, $t_1 = 4$, $P_1 = " " ?$.
  ...
  (
    statement: [
      Pair the differential equation with a slope field and pair that slope field with a graph of
      the solution for the following differential questions/slope fields/ functions.
          ...
          align(
            ...
            slope_field(
              ...
              yaxis: (
                position: 0,
                tip: tiptoe.stealth,
                filter: ((v, d) => calc.round(v, digits: 2) in (1., 2., 3., 4.)),
              ),
              xaxis: (
                position: 0,
                tip: tiptoe.stealth,
                filter: ((v, d) => calc.round(v, digits: 2) in (0, 1., 2., 3., 4.)),
              ),
              xlabel: $t$,
              ylabel: lq.label($y$),
            ),
  ...
  (
    statement: [
      A sample of Uranium is decaying. Make the following assumptions about the decay of the Uranium
      sample:
      - When the sample if first observed, it has a mass of 1 kilogram.
      - Initially the sample is decaying at a rate of $1.55 times 10^(-10)$ kilograms per year.
      - The rate of decay is proportional to the mass of the sample.
      + Build a differential equation that pairs with these assumptions to make model for the mass
        of the sample.
      + Use Euler's method in Excel, or an equivalent software, with a step size of
        $1 "million years"$ to construct a graph that approximates a solution to this differential
        equation.
  ...
  (
    statement: [
      ...
      + With $Delta = 0.1 "hr"$ and going up to at least $t = 20"hr"$, use Euler's method to produce
        a graph of an approximation of a solution the differential equation you wrote in part (a).
        Use this to make a prediction about how high the pilot will go.
      + Repeat part (b) with $Delta = 0.02 "hr"$. Again make a prediction of how high the pilot will
        go.
      + If the previous two parts were done correctly, your predictions should differ greatly. This
        difference is too great to be described solely by the normal "make $Delta$ small"
        explanation. Describe in one or two sentences what about this differential equation, and
        making "$Delta$ small", is causing such a great deviation in the results of Euler's method
        in (b) and (c). (Hint: Try making a slope field around the height you think the maximum
        height should be in part (c).)
    ],
  ),
  (
    statement: [
      ...
      disposal to determine which of them has
      the smaller Delta and give an explanation
      as to why the one you chose is
      has the smaller delta.*/
      A physicist named John is trying using a differential equation to come up with graph that
      represents the mass of space around a Black hole. where $x$ is light hours, in $"ly-h"$,
      /*seconds? years? days?*/
      from the black hole, and $m$ is the mass of the black hole, in solar masses $"M"$. The
      differential equation he is looking at has the following slope field:
      #let F(x, y) = -3 / x
      #{
        align(
          center,
          slope_field(
            ...
            yaxis: (
              position: 0,
              tip: tiptoe.stealth,
              filter: ((v, d) => calc.round(v, digits: 2) in (5, 10, 15)),
            ),
            xaxis: (
              position: 0,
              tip: tiptoe.stealth,
              filter: ((v, d) => calc.round(v, digits: 2) in (-2, -1, 0, 1, 2)),
            ),
            xlabel: $x$,
      ...
      John's coworker has made with Euler's method two graphs, which are below, that are the same
      except for one graph has a smaller $Delta$ than the other. Use all you have learned so far to
      determine which of the graph John is looking as has the smaller step size.
      + #{
          let soln = solve_1d_ivp(
            F,
            (
  ...
  (
    statement: [
      ...
      */
      In this question the idea of using Euler's method on a multidimensional system of differential
      equations is introduced. Consider the system of differential equations
      $
        x' = y \ y' = x
      $
      where $x = f(t)$ and $y = g(t)$ are both functions of $t$.
      + Assume $(t_0,x_0,y_0) = (0,0,1)$ is the initial condition of this differential equation. Use
      ...
      + Enter all your data into a table with $5$ columns. Continue until you have
        ...
        $y'|_(t=t_4) = g'(t_4)$
      + Enter all this data into a program similar to Excel such that all calculations are computed
        automatically. What is the value of $(t_(365),x_(365),y_(365))$.
    ],
  ),
  (
    statement: [
      True or false:
      + A differential equation $f(y,y',t) = 0$ is autonomous if and only if on any slope field of
        that differential equation each $y$ value has only one slope value $y'$, independent of $t$.
      /*I am a little worried about the
      phrasing here to make it strictly true*/
      + Since particular solutions to a \
        differential equation differ by a constant, they are all parallel to each other on a slope
        field.
      + There is a differential equation Eulers method is exactly accurate at all points no matter
        what the step size is.
  ...
  (
    statement: [
      Bonus questions:
      + Is the Excel graph in your solution to question 3 (c), consistent with the slope field that
        you drew in question 1 (c)? If so, is this particular solution special in some way that you
        can describe with respect to the differential equation? If not, why is it inconsistent?
      + This question is a continuation of question 6:
        + The differential equation that you came up with in question 6 (a) is most likely
          separable. Use this to find the solution to your differential equation
        + Use that solution to find exactly the half-life of the sample of Uranium. How close is
          your approximation to the true value?
        + Would it have been possible to find the half-life without the solution to the differential
          equation? In this case, perhaps you could use only the differential equation itself and
          not its solution.
        //Not sure this is possible.
        + Get the solution approximation with Euler's method. This should be done by hand this time,
          so there must be a way to incentivize this.
        + Graph the solution you just found by hand and use desmos to graph the solution. Confirm
          that these graphs the same, at least relative to their accuracy.
      + In question 9, make a graph of $y$ as a function of $x$. What shape do you get? Write a
        sentence or two describing how and why changing the value of $Delta$ changes this shape.
        What would the shape look like if you could make delta infinitely small?
    ],
  ),
)
