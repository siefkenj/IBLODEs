...
\begin{itemize}
	...
	\item $\vec{T}_R = \beta \vec{T}_R$ for some $\beta >0$.
\end{itemize}


\begin{parts}

\item Can you find approximations of the vectors $\vec{F}_g, \vec{T}_L, \vec{T}_R$ that only use $y(x)$, $y'(x)$, $y''(x)$?
...
\item Put everything together to find a (second order) differential equation for $y$.
...
\begin{parts}
	...
	\item Toronto Hydro is stringing some wire. The posts are 100m apart and at a height of 20m. At the lowest point, the wire is 13m above the ground.
