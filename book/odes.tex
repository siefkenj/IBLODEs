...
Consider a rope hanging from two poles at positions $x=0$ and $x=1$.
...
\begin{tikzpicture}
  ...
  \draw[dashed,gray] (0,-0.12) node[below] {\scriptsize $x$} -- (0,1);
  \draw[dashed,gray] (0.5,-0.12) node[below] {\scriptsize $\quad x+\Delta$} -- (0.5,{cosh(0.5)});
\end{tikzpicture}
\end{center}
\vspace{-1em}
$y(x)=$ the height of the rope from the ground at position $x$.

We will consider the following premises and physics laws:
\vspace{-1em}
\begin{itemize}
	\item[(D)] The linear density of the rope is constant: $\rho$ kg/m

	\item[(G)] Gravity pulls downwards proportionally to the mass (the proportionality constant is called $g$)
	
	\item[(T)] Tension pulls tangentially to the rope

%	\item[(S)] The rope is not moving (it is stationary)

	\item[(NL)] Newton's First Law: a body at rest will remain at rest unless it is acted upon by a force
...
We will focus on one such rod $S$, as in the figure, from $x$ to $x+\Delta$.

\begin{parts}

\item Given (NL), find a relation between the force vectors $\vec{T}_L, \vec{T}_R, \vec{F}_g$.
...
Similarly to the previous exercise, we can find a vector $\vec{V}_R = \big(1,y'(x+\Delta)\big)$ in the direction of $\vec{T}_R$, but with possibly different magnitude.

We can approximate:
	\begin{itemize}
		\item $y(x+\Delta) \approx y(x) + \Delta \cdot y'(x)$,
		\item $y'(x+\Delta) \approx y'(x) + \Delta \cdot y''(x)$.
	\end{itemize}

Observe the following:
\begin{itemize}
	\item $\vec{V}_L = a_L \vec{T}_L$ and $\vec{V}_R = a_R \vec{T}_R$, for some $a_L, a_R >0$.
\end{itemize}


\begin{parts}

\item Can you find approximations of the vectors $\vec{F}_g, \vec{T}_L, \vec{T}_R$ that only use the values of the function $y$ at $x$ ($y(x), y'(x), y''(x), \ldots$) and not at $x+\Delta$?

\item Put everything together to find a (differential) equation for $y$.
...
Recall a rope hanging from two poles at positions $x=0$ and $x=1$.
...
\[
y''(x) = \frac{\rho g}{T} \sqrt{1+\big(y'(x)\big)^2}
\]
\vspace{1pt}

\begin{parts}

	\item Toronto Hydro is stringing some wire. The posts are 100m apart and at a height of 20m. At their lowest point, the wire is 13m above the ground.
	
	The wires have a (linear) density of 1 kg/m and gravity is 9.8 m/s$^2$.
