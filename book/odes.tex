...
Similarly to the previous exercise, we can find a vector $\vec{V}_R = \matc{1\\y'(x+\Delta)}$ in the direction of $\vec{T}_R$, but with possibly different magnitude.
...
\item Do $\alpha$ or $\beta$ depend on $x$? Explain.
